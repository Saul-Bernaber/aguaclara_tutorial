...
## Same as Interactive Tutorial for Hydrogen?
...
3. For the remaining code, highlight it with your cursor and use `Hydrogen: Run`. What is the difference between the three? The difference is that one of them imports a python package, the second one makes an array which is then plotted.
...
from aguaclara.play import*
import math as m
T = float(input("Give me Temp")) * u.kelvin
n = float(input("Give me Eta")) * u.kg / (u.m * u.s)
r = float(input("Give me radius")) * u.m
D =(kB*T)/(6*m.pi*n*r)
print(D)
```
...
xxArray = u.Quantity(np.arange(0.1, 200, 1), u.kelvin)
...
plt.plot(xxArray, re_flat_plate(2 * u.m**2/u.s, .2 * u.m, pc.viscosity_kinematic(293 * u.kelvin))), '-', label = 'Blasius Solution')
plt.xlabel('Temperature (Kelvin)')
plt.ylabel('Reynolds Numbers (NA)')
plt.title('Temperature Vs Reynold')
plt.minorticks_on()
plt.grid(which = 'major')
plt.grid(which = 'minor')
plt.legend(loc = 'lower right', ncol = 1)
